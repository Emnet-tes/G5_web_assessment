'use client';
import { Blog, getBlogs, relatedBlogs } from "@/app/lib/api";
        ...
        <div>
          {filteredBlogs.length > 0 ? (
            filteredBlogs.slice(0, 2).map((blog) => (
              <div
                key={blog._id}
                className="mb-4 p-4 border rounded absolute bg-gray-700 opacity-50 text-white"
              >
            ...
            <p className="text-gray-300">No blogs found matching your search criteria.</p>
          )}
        </div>
      </div>
    </div>
